...
class cl_statusBar_frame( tk.Frame ) :
  ...
  def set( self, formatStr, *args ) :
    self.label.config( text = "kpg3522: "+formatStr % args )
...
class cl_toolbar :
  ...
  def toolbar_load_callback( self ) :
    ...
    if(len(fName) != 0):
      self.mymodel = ModelData.ModelData(fName)
  ...
  def toolbar_draw_callback( self ) :
    if self.mymodel:
      ...
      print("Canvas Size: ({},{})".format(width,height))
      w = self.mymodel.getWindow()
      v = self.mymodel.getViewport()
      print("Window: ",w)
      print("Viewport: ", v)
      (ax, ay, sx, sy) = constructTransform.constructTransform(w, v, width, height)
      self.mymodel.specifyTransform(ax, ay, sx, sy)
      print("Transform : ax {} ay {} sx {} sy {}".format(round(ax,3),round(ay,3),round(sx,3),round(sy,3)))
      self.master.ob_world.create_graphic_objects( self.master.ob_canvas_frame.canvas, self.mymodel )
      Lefttop = (width * v[0], height * v[1])
      Righttop = (width * v[2], height * v[1])
      Leftbottom = (width * v[0], height * v[3])
      Rightbottom = (width * v[2], height * v[3])

      self.master.ob_canvas_frame.canvas.create_line(Lefttop[0], Lefttop[1], Righttop[0], Righttop[1])
      self.master.ob_canvas_frame.canvas.create_line(Lefttop[0], Lefttop[1], Leftbottom[0], Leftbottom[1])
      self.master.ob_canvas_frame.canvas.create_line(Righttop[0], Righttop[1], Rightbottom[0], Rightbottom[1])
      self.master.ob_canvas_frame.canvas.create_line(Leftbottom[0], Leftbottom[1], Rightbottom[0], Rightbottom[1])
